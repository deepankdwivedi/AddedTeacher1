...
public class CalendarAdapter extends BaseAdapter {
	...
	public 	CalendarAdapter(Context getCalendarView, GregorianCalendar monthCalendar,
						   ArrayList<String> present_dates) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		for (int i = 0; i < CalendarCollection.present_dates.size(); i++) {
			if (day_string.get(position).equals(
					CalendarCollection.present_dates.get(i))) {
		...
		for (int i = 0; i < CalendarCollection.holiday_dates.size(); i++) {
			System.out.println("Calendar Holiday:"+CalendarCollection.holiday_dates.get(i).holidayDate);
			if (day_string.get(position).equals(
	...
	public void setEventView(View v, int pos, TextView txt) {
		...
		int len = CalendarCollection.holiday_dates.size();
		for (int i = 0; i < len; i++) {
			CalendarCollection cal_obj = CalendarCollection.holiday_dates
					.get(i);
			String date = cal_obj.holidayDate;
			String message = cal_obj.message;
	...
	public void getPositionList(String date, final Activity act, final View vi) {
		...
		int len = CalendarCollection.holiday_dates.size();
		for (int i = 0; i < len; i++) {
			CalendarCollection cal_collection = CalendarCollection.holiday_dates
					.get(i);
			String event_date = cal_collection.presentDate;

			String event_message = cal_collection.message;
